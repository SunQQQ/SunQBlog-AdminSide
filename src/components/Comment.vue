      ...
      <el-table :data="CommentList" style="width: 100%" border :header-cell-style="{ background: '#f7f7f7' }">
        ...
        <el-table-column fixed="right" label="操作" width="130">
          <template slot-scope="scope">
            ...
            <el-button @click="DeleteComment(scope.row.id)" type="text" size="small" class="danger-color">删除</el-button>
          </template>
        </el-table-column>
      </el-table>

      <!--分页器-->
      <el-pagination layout="total,prev, pager, next" :total=CommentTotal :page-size=PageSize @current-change="GetData"
        @next-click="GetData" @prev-click="GetData" v-if="CommentTotal > 0">
      </el-pagination>
    </div>
  </main>
</template>
